...
def test_compile():
    ...
    sdb = backend.stencil_db_from_package('riscv32')
    ...
    dw.to_file('build/runner/test-riscv32.copapy')

    if not check_for_qemu():
        warnings.warn("qemu-riscv32 not found, riscv32 test skipped!", UserWarning)
        return
    if not os.path.isfile('build/runner/coparun-riscv32'):
        warnings.warn("riscv32 runner not found, riscv32 test skipped!", UserWarning)
        return

    command = qemu_command + ['build/runner/coparun-riscv32', 'build/runner/test-riscv32.copapy'] + ['build/runner/test-riscv32.copapy.bin']
    #try:
